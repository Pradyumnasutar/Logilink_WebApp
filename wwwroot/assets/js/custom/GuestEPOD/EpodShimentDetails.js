...
$(document).ready(function () {
    ...
    SetBreacrumbColors();
    InitialiseTable(pathname + "/GuestEPOD/GetDeliveryOrderListforEpod");
    LoadAttachments(pathname + "/GuestEPOD/GetOutboundShipmentDocuments");
    uploadAttachment();
    uploaddoAttachment();
    //Tabs Slider
    ...
    $('#btnPrintShipmentOrder').click(function () {
        $('#printAuthenticateModal').modal('show');
    });
    ...
    $('#confirmPrint').click(function () {
        PrintAuthenticateShipmentOrder();
        $('#printAuthenticateModal').modal('hide');
    });
    ...
    $(document).on('click', '.DO_DETAILS', function (e) {
        ...
        if (Doid > 0) {
            ...
            IntialiseDoLines(pathname + "/GuestEPOD/GetDeliveryOrderLinesByDeliveryOrder", Doid);
...
function LogOut() {
    NavigatePage("/GuestEPOD/EpodLogout");
}
//#region Print Document

function PrintAuthenticateShipmentOrder() {
    ...
    $.ajax({
        type: 'POST',
        url: pathname + "/GuestEPOD/PrintShipmentOrders",
        data: {
...
function GenerateGoodsReturnReport(shipmentid) {
    ...
    $.ajax({
        type: 'POST',
        url: pathname + "/GuestEPOD/PrintGoodsReturnReport",
...
function populateTabs() {
    ...
    var deliveryOrders = Ajax(pathname + "/GuestEPOD/GetDeliveryOrderListforEpodTabs", { "ShipmentId": ShipmentId }, true);
...
function ActivePopupTab(doid) {
    ...
    IntialiseDoLines(pathname + "/GuestEPOD/GetDeliveryOrderLinesByDeliveryOrder", actualdoiid);
...
function DownloadOutboundAttachment(documentId) {
    ...
    $.ajax({
        url: pathname + "/GuestEPOD/DownloadshipmentAttachement",
...
function DownloadDeliveryOrderAttachment(deliveryDocumentId, DOId) {
    ...
    $.ajax({
        url: pathname + "/GuestEPOD/DownloadOrderAttachment",
...
function DownloadAttachment(deliveryDocumentId, DOId) {
    ...
    if (overlay) {
        overlay.style.position = 'fixed';
        $('.blockui-overlay').css('z-index', '3000');
...
function loadGoodReturnCodelist() {
    $.ajax({
        url: pathname + '/GuestEPOD/GetgoodreturnReasonlist',
        type: 'POST',
    ...
    if (remark != null && remark != '') {
        try {
            if (Model.shipmentid > 0) {
                $.ajax({
                    url: pathname + '/GuestEPOD/SaveEpodRemark',
                    type: 'POST',
                    data: { shipmentid: Model.shipmentid, remarks: remark },
                    success: function (response) {
                ...
                "data": "grn_reason_code", "name": "Reason Code", "autoWidth": true, "render": function (data, type, full, meta) {
                    if (Model.shipmentInfo.shipment_statusid == 8) {
                        if (full.grn_reason_id != undefined && full.grn_reason_id > 0) {
                            ...
                            return ' <select data-placeholder="Select return code" data-hidden-id="' + full.grn_reason_id + '" data-id="' + full.delivery_order_lines_id + '" data-control="select2" class="form-select fs-7 return-code">' + options +'</select > '

                        }
                        else {
                            return ' <select data-placeholder="Select return code" data-id="' + full.delivery_order_lines_id + '" data-control="select2" class="form-select fs-7 return-code">'+nonoptions+'</select > '
...
function UploaddoDoc(Doid) {
    ...
    if (file && Doid) {
        ...
        if (isValidFileType(file.name)) {
            ...
            $.ajax({
                url: pathname + '/GuestEPOD/UploaddoAttachments',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
...
function UploadDoc() {
    ...
    if (file && shipmentId) {
        if (isValidFileType(file.name)) {
            ...
            // AJAX request
            $.ajax({
                url: pathname + '/GuestEPOD/UploadAttachments',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
...
function RemoveDocfromtable(docid, filename) {
    ...
    try {
        if (filename && shipmentId && docid) {
            $.ajax({
                url: pathname + '/GuestEPOD/DeleteAttachment',
                type: 'POST',
...
function RemovedeliveryOrderDocfromtable(docid, deliveryorderid) {
    ...
    try {
        if (docid>0) {
            $.ajax({
                url: pathname + '/GuestEPOD/RemoveDeliveryOrderDocumnet',
                type: 'POST',
...
function SaveDeliveryorderLines(shipmentid, deliveryorderid, DOremark, Dolines) {
    if (shipmentid > 0 && deliveryorderid > 0 && Dolines != undefined) {
        ...
        $.ajax({
            url: pathname + '/GuestEPOD/SaveDeliveryOrderLines',
            type: 'POST',
            ...
            success: function (response) {
                ...
                if (response.result) {
                    Swal.fire({
                        ...
                        IntialiseDoLines(pathname + "/GuestEPOD/GetDeliveryOrderLinesByDeliveryOrder", deliveryorderid);
                        InitialiseTable(pathname + "/GuestEPOD/GetDeliveryOrderListforEpod");
...
function GetOrderAttachment(deliveryOrderId) {
    ...
    var gh = 0;
    var count = 0;
    var _url = pathname + "/GuestEPOD/GetOrderAttachment";
    var isaccess = false;
    if (slModuleAction[3] > AccessLevels.Write) {
        if (Model.shipmentInfo.shipment_statusid != 10) {
            isaccess = true;
    ...
    shipdoctable = $("#tbldoDocuments").DataTable({
        ...
        "columns": [
            ...
            {
                "data": "", "name": "Attachments", "autoWidth": true, "render": function (data, type, full, meta) {
                    return "<a href='#'class='btnDownloadDO' data-docid=" + full.deliveryDocumentId +" data-id=" + full.deliveryOrderId + ">" + full.documentName + "</a>"
                    //return full.documentName;
                }
            },
            {
