...
function Initialize() {
    ...
    $('#menuVessels .menu-link').addClass("active");
    $('#btnexportVessels').click(function () {
        PrintTransportType();
    });
    var isaccess = false;
...
function AddNew(e) {
    ...
    oTable = oTable.row.add(['0',
        ...
        '<input type="text" class="form-control fs-7" id="txtimono" maxlength="250" placeHolder="IMO"/>',
...
function editRow(id) {
    ...
    jqTds[2].innerHTML = '<input type="text" class="form-control fs-7" id="txtimono" maxlength="250" placeHolder="IMO" value="' + Str(aData[3]) + '"/>';
    jqTds[3].innerHTML = detTag;
}

function SaveRow() {
    var code = $('#txtCode').val();
    var descr = $('#txtDescr').val();
    var imo = $('#txtimono').val();
    if (code != '' && descr != '') {
        $.ajax({
            type: "POST", async: false,
            url: pathname + "/Masters/SaveVessels",
            data: { "id": UOMID, "Vessel_Code": code, "Vessel_Name": descr, "imo_no": imo },
            success: function (response) {
                try {
                    if (response.result == true) {
                        var msg = 'Vessels added successfully!';
                        if (UOMID > 0) msg = 'Vessels updated successfully!';
                        ...
                        if (response.msg != '') {
                            ...
                            Swal.fire('Error', 'Unable to save Vessels', 'error');
                        }
                    }
                }
                catch (err) {
            ...
            error: function (response) {
                ...
                if (response.responseText.indexOf("Violation of UNIQUE KEY") > 0) {
                    Swal.fire('Unable to save Vessels', "Please enter unique code", 'error');
                }
                else Swal.fire('Unable to save Vessels', response.responseText, 'error');
            }
        });
    }
...
function PrintTransportType() {
    ...
    $.ajax({
        type: 'POST',
        url: pathname + "/Masters/ExportVessels",
        data: {
